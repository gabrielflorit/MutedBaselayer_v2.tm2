...
//@land: black;
@whitener: 100%;
@water: mix(#cdd, @land, @whitener);
@water_dark: mix(#185869, @land, @whitener);  // for the inline/shadow
@crop: mix(#eeeed4, @land, @whitener);
@grass: mix(#e7ebd1, @land, @whitener);
@scrub: mix(#e0e8cd, @land, @whitener);
@wood: mix(#d4e2c6, @land, @whitener);
@snow: #f4f8ff;
@rock: #ddd;
@sand: mix(#ffd,@land,33%);
// These colors need to take `comp-op:multiply` into account:
@cemetery: #edf4ed;
@pitch: fadeout(#fff, @whitener);
@park: mix(#edf9e4, @land, @whitener);
